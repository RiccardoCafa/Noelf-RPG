                    ...
                    case "legs":
                        foreach (string side in sides)
                        {
                            ...
                            for (int bit = 0; bit < 2; bit++)
                            {
                                ...
                                playerImages[parts[i] + side + bit].Source = new BitmapImage(new Uri(MainPage.obj.BaseUri, path1 + path2 + path3));
                            }
                        }
                        break;
                    case "hair":
                        ...
                        playerImages[parts[i]].Source = new BitmapImage(new Uri(MainPage.obj.BaseUri, path1 + path2));
                        break;
                    case "eye":
        ...
        private void SetClothes(Dictionary<string, Image> clothesImages)//aplica as imagens das roupas do player (classe)
        {
            for (int i = 3; i < 6; i++)
            {
                ...
                if (parts[i] == "arms" || parts[i] == "legs")
                {
                    foreach (string side in sides)
                    {
                        ...
                        for (int bit = 0; bit < 2; bit++)
                        {
                            ...
                            clothesImages[parts[i] + side + bit].Source = new BitmapImage(new Uri(MainPage.obj.BaseUri, path1 + path2 + path3));
                        }
                    }
                }
                else
                {
                    ...
                    clothesImages[parts[i]].Source = new BitmapImage(new Uri(MainPage.obj.BaseUri, path1 + path2));
                }
            }
        }
